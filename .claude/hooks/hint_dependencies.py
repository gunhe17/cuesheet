...
목적: LLM 이 직접 grep/CLI 로 의존성을 뒤지지 않고 MCP(`show_layering`)를 무조건 거치게 강제.
검사 로직은 MCP 도구(cuesheet/mcp/tools/graph)가 단일 소유. 훅은 HTTP 위임만.
...
async def fetch_dependencies(url: str, file: str) -> dict:
    ...
    async with streamablehttp_client(url) as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            result = await session.call_tool("show_layering", {"file": file})
...
def run() -> int:
    ...
    lines.append("위→아래 import만 허용(모두 →core). 위반 import 를 제거하거나 의존을 뒤집어라. cuesheet/api/CLAUDE.md [INV-1] 참고.")
