...
검사 로직은 MCP 도구(`show_usages`, cuesheet/mcp/tools/graph)가 단일 소유.
...
async def fetch_dependents(url: str, file: str) -> dict:
    ...
    async with streamablehttp_client(url) as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            result = await session.call_tool("show_usages", {"file": file})
    ...
    data = data or {}
    if "usages" not in data and isinstance(data.get("result"), dict):
        data = data["result"]
    return data
...
def run() -> int:
    ...
    if not path.endswith(".py") or "/cuesheet/api/" not in normalized:
        return 0
    ...
    shown = usages[:CAP]
    lines = [f"[blast radius] {os.path.basename(path)} 를 import하는 곳 {len(usages)}개 — 수정 시 함께 반영:"]
    lines.extend(f"  - {d}" for d in shown)
    if len(usages) > CAP:
        lines.append(f"  ... 외 {len(usages) - CAP}개")
