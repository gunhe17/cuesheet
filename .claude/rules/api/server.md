...
루트: [api/CLAUDE.md](../../../cuesheet/api/CLAUDE.md) · endpoint: [endpoint.md](endpoint.md) · 예외: [exception.md](exception.md)
...
> 온톨로지 · essence: 어댑터의 본질 = 도메인 무지(`Callable`만, [INV-1]) — 도메인을 알면 "어댑터"라는 종류가 아니다. → [ontology.md](../../../cuesheet/api/.claude/documents/ontology.md)

도메인을 모르는 순수 어댑터 — `Callable` endpoint를 받아 FastAPI에 꽂는다.

| 파일 | 내용 |
|------|------|
| `server/server.py` | `Server` — 등록 큐(`middleware`/`router`/`lifecycle`/`exception_handler`) + `app()`(전부 `register(app)` 호출). `cuesheet_api()` 팩토리 |
...
from cuesheet.api.endpoint import secret      # 모듈 namespace import
