...
루트: [api/CLAUDE.md](../../../cuesheet/api/CLAUDE.md) · 핸들러 등록: [server.md](server.md)
...
> 온톨로지 · taxonomy: 상위 범주가 처리를 결정 — ClientError(클라 책임)/DevelopError(서버 책임)라는 "누구 책임"이 핸들러 분기(MRO)를 만든다. → [ontology.md](../../../cuesheet/api/.claude/documents/ontology.md)
...
- 미처리 예외는 `internal()` catch-all — 단 raw 오류는 경계(어댑터)에서 typed 변환. 외부 lib 예외를 그 어댑터 typed로(`SQLAlchemyError → DatabaseError`, `Argon2Error → VerifyError`). 외부 시스템(DB·hash)을 만지는 어댑터는 전부 typed 경계를 갖는다 — raw 전파 0
