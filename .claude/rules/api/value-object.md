---
paths:
  - "cuesheet/api/domain/**/*.py"
  - "cuesheet/api/core/value_object.py"
---
...
루트: [api/CLAUDE.md](../../../cuesheet/api/CLAUDE.md) · 공통 스타일: [conventions.md](../shared/conventions.md) · 베이스: [repository.md](repository.md)는 별개
...
> 온톨로지 · identity: 값은 구조로 동일(내용이 같으면 같음) — `by_factory`는 검증된 구성만 그 값이 되게 하는 동일성 게이트. Entity(참조 동일성)와 다른 종류. → [ontology.md](../../../cuesheet/api/.claude/documents/ontology.md)
...
> 온톨로지 · identity: 도메인 값 = 검증된 종류라 VO. raw primitive는 종류를 흐린다. UUID·audit만 예외 — 이미 강타입/프레임워크 필드라 감싸도 종류가 안 는다. → [ontology.md](../../../cuesheet/api/.claude/documents/ontology.md)
