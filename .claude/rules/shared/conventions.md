...
`cuesheet/` 전 앱(api·cli·mcp…) 공통 Python 코드 스타일. 레이어/앱별 패턴은 각 규칙 문서를 참고 — 이 문서는 그와 무관한 시각/포맷 규칙만 담는다.

적용 범위: `cuesheet/` 전 앱 공통 (api 레이어 패턴은 [rules/api/](../api/)).
...
순서: 표준 → 서드파티 → 로컬. 그룹마다 한 줄 공백. 로컬은 패키지 루트부터 절대경로 `from cuesheet.api.xxx`. 도메인 클래스는 한 줄에 하나씩 별도 import.
...
from cuesheet.api.domain.secret.name import Name
from cuesheet.api.domain.secret.kind import Kind
from cuesheet.api.domain.secret.tags import Tags
```
...
from cuesheet.api.endpoint import secret
...
from cuesheet.api.endpoint.secret import post_create as secret_post_create
```
